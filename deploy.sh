...
create_lambda_infrastructure() {
  ...
  ( cd ./infras/lambda_infra && terraform init -backend-config="bucket=$bucket_name" && terraform apply -auto-approve -var "scrape_url=$SCRAPE_URL" -var "aws_region=$aws_region" )
  if [ $? -ne 0 ]; then
